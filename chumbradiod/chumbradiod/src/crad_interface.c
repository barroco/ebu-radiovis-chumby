...
int crad_create(struct _crad_info_t *p_crad_info, struct _crad_t **pp_crad)
{
    ...
    *pp_crad = p_crad;
    ...
    QND_SetCountry(COUNTRY_EUROPE);

    fprintf(stderr, "Setting audio mode...\n");
    QNF_SetAudioMono(0x38, QND_RX_AUDIO_MONO);
    //QND_WriteReg(TX_FDEV, 0x40); //MC, Tweak deviation
    QNF_SetRegBit(SYSTEM2,0x08, 0x00); // FM time constant 00=50us 08=75us (second argument)
...
int crad_get_status_xml(struct _crad_t *p_crad, char *xml_str, int max_size) {
    ...
    if(!p_crad->rds_thread_running) {
		...
		data_copy.program_identification,
		data_copy.extended_country_code
        );
    }
